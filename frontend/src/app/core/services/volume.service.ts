...
import { Volume, CreateVolume, VolumeResponse } from "../interfaces/volume";
...
export class VolumeService {
  ...
  private readonly _VOLUMES = "volumen/records";
  constructor(private _http: HttpClient, private _toastr: ToastrService) {}

  getVolumes(): Promise<VolumeResponse> {
    return new Promise((resolve, reject) => {
      this._http
        .get<VolumeResponse>(this._URL + this._VOLUMES)
        .pipe(
          take(1),
          catchError((error) => {
  ...
  createVolume(register: CreateVolume): Observable<any> {
    console.log(register);
    let url = `${this._URL}${this._VOLUMES}`;
    return this._http.post<ActionResponse>(url, register).pipe(
      ...
      catchError((error) => {
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "There was an error creating the volume",
        });
        this.errorHandle(error);
        return EMPTY;
      }),
  ...
  updateVolume(register: Volume): Observable<any> {
    let url = `${this._URL}${this._VOLUMES}/${register.id}/`;
  ...
  deleteVolume(id: Number): Observable<any> {
    let url = `${this._URL}${this._VOLUMES}/${id}/`;
